 ...
 */
import { aroundList, elementCentre, haversine, overpass, project, simplify, wikiNearLine, } from "@slownav/core";
 ...
 * walk over.
 */
export async function fetchSights(pts, onStatus) {
    const coarse = aroundList(simplify(pts, CORRIDOR_TOLERANCE));
    const near = (r) => `(around:${r + CORRIDOR_TOLERANCE},${coarse})`;
 ...
 */
export function classifySight(t) {
    const of = (group, kind, icon) => ({ group, kind, icon });
    if (t.natural === "peak") {
        return of("peak", t.ele ? `peak · ${Math.round(+t.ele)} m` : "peak", "peak");
    }
    if (t.natural === "saddle")
        return of("peak", "col / pass", "peak");
    if (t.natural === "spring" ||
        t.amenity === "drinking_water" ||
        t.amenity === "fountain" ||
        t.man_made === "water_well") {
        const kind = t.amenity === "drinking_water"
            ? "drinking water"
            : t.natural === "spring"
                ? "spring"
                : "fountain";
    ...
    if (t.natural === "cave_entrance")
        return of("sight", "cave", "poi");
    if (t.natural === "waterfall" || t.waterway === "waterfall")
        return of("sight", "waterfall", "poi");
    if (t.natural === "cliff" || t.natural === "arch")
        return of("sight", t.natural, "poi");
    if (t.amenity === "shelter" || t.tourism === "alpine_hut" || t.tourism === "wilderness_hut") {
        const kind = t.tourism === "alpine_hut" ? "refuge" : t.tourism === "wilderness_hut" ? "hut" : "shelter";
        return of("hut", kind, "hut");
    }
    if (t.tourism === "viewpoint")
        return of("sight", "viewpoint", "poi");
    if (t.aerialway)
        return of("sight", "cable car", "poi");
    if (t.tourism === "camp_site")
        return of("service", "campsite", "poi");
    if (/^(hotel|guest_house)$/.test(t.tourism || "")) {
        return of("service", t.tourism.replace("_", " "), "poi");
    }
    if (t.tourism === "picnic_site")
        return of("service", "picnic site", "poi");
    ...
    if (t.amenity === "toilets")
        return of("service", "toilets", "poi");
    if (t.amenity === "pharmacy")
        return of("service", "pharmacy", "poi");
    if (/^(restaurant|cafe|bar|pub)$/.test(t.amenity || ""))
        return of("food", t.amenity, "poi");
    if (t.shop)
        return of("shop", t.shop.replace("_", " "), "poi");
    if (t.amenity === "place_of_worship")
        return of("sight", "church", "poi");
    if (t.historic)
        return of("sight", t.historic.replace(/_/g, " "), "poi");
    if (t.man_made === "cross")
        return of("sight", "cross", "poi");
    if (t.man_made)
        return of("sight", t.man_made, "poi");
    if (t.tourism)
        return of("sight", t.tourism.replace(/_/g, " "), "poi");
    if (t.place)
        return of("place", t.place, "poi");
    return null;
}
/** Articles near the track, English first, Spanish where nothing English exists. */
export function fetchWiki(pts, signal) {
 ...
 */
function maxLateral(group) {
    if (group === "water" || group === "hut")
        return 350;
    if (group === "peak")
        return 900;
    return 800;
...
export function buildSights(elements, wiki, track) {
    const sights = [];
    const seen = new Set();
    const place = (lat, lon) => project(track.pts, track.cum, [lat, lon], { global: true });
    for (const e of elements) {
        ...
        if (!tags.name && c.group !== "water" && c.group !== "hut")
            continue;
        const p = place(centre[0], centre[1]);
        if (!p || p.dist > maxLateral(c.group))
            continue;
    ...
    for (const w of wiki) {
        ...
        const near = sights.find((s) => haversine([s.lat, s.lon], [w.lat, w.lon]) < 200 ||
        ...
        if (near) {
            ...
            if (!near.wiki || (near.wiki.lang !== "en" && w.lang === "en"))
                near.wiki = article;
        }
        else {
            sights.push({
