 ...
 */
import { applyFix, cachedCount, corridorTiles, hikeTracker, loadSession, newSession, positionAt, saveSession as persistSession, store, HIKE_MATCH, } from "@slownav/core";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { APP, eta, library, prepare, waypointsAlong } from "./model.js";
import { buildSights, fetchSights, fetchWiki } from "./sights.js";
const INITIAL = {
    hike: null,
    track: null,
    waypoints: [],
...
export function useHike(settings) {
    ...
    const openHike = useCallback((id) => {
        ...
        const sights = store.get(`hike:sights:${id}`);
        ...
        if (session)
            tracker.current.anchor(progress);
        setState({
            ...INITIAL,
            hike,
            track,
    ...
    const setPreview = useCallback((metres) => {
        setState((s) => {
            if (!s.track)
                return s;
    ...
    const loadSights = useCallback(async (hike, track) => {
        ...
        try {
            ...
            if (stateRef.current.hike?.id === hike.id)
                patch({ sights, sightsStatus: "ok" });
        }
        catch {
    ...
    const currentEta = useMemo(() => (state.track ? eta(state.track, state.progress, state.session) : null), [state.track, state.progress, state.session]);
    return {
        state,
        patch,
        eta: currentEta,
...
//# sourceMappingURL=useHike.js.map